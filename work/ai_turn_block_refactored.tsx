// ui/components/AiTurnBlock.tsx - CHILDREN COMPOSITION PATTERN
import React from 'react';
import { AiTurn, AppStep } from '../types';
...
interface AiTurnBlockProps {
  ...
  children?: React.ReactNode;  // ✅ NEW: Accept children for composition
}
...
const AiTurnBlock: React.FC<AiTurnBlockProps> = ({
  ...
  onClipClick,
  children,  // ✅ NEW: Destructure children prop
}) => {
  ...
  const getSynthesisAndOptions = useCallback((take: any) => {
  ...
  /**
   * ✅ CRITICAL: Determine if we have sources to show
   * This logic used to be duplicated in AiTurnBlockConnected
   */
  const hasSources = useMemo(() => {
    const allSources: Record<string, any> = { ...(aiTurn.batchResponses || {}) };
    if (aiTurn.hiddenBatchOutputs) {
      Object.entries(aiTurn.hiddenBatchOutputs).forEach(([providerId, response]) => {
        if (!allSources[providerId]) {
          allSources[providerId] = response;
        }
      });
    }
    return Object.keys(allSources).length > 0;
    ...
    <div className="turn-block" style={{ paddingBottom: '1rem', borderBottom: '1px solid #334155' }}>
      ...
      <div className="ai-turn-block" style={{ border: '1px solid #334155', borderRadius: 12, padding: 12 }}>
        <div className="ai-turn-content" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {/* Synthesis and Mapping sections - unchanged from original */}
          <div className="primaries" style={{ marginBottom: '1rem', position: 'relative' }}>
            <div style={{ display: 'flex', flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
              ...
              >
                {/* ... synthesis rendering logic unchanged ... */}
              ...
              {/* Mapping Section - similar structure */}
              <div 
                ref={mapRef}
                className="mapping-section" 
                style={getSectionStyle('mapping', mapExpanded)}
              >
                {/* ... mapping rendering logic - keeping original complexity for now ... */}
              </div>
            </div>
          </div>

          {/* 
            ✅ CRITICAL COMPOSITION POINT: Render children here
            
            This is where ProviderResponseBlockConnected will be inserted
            by AiTurnBlockConnected. This allows the source outputs to have
            their own isolated subscription to provider responses.
          */}
          {hasSources && (
            <div 
