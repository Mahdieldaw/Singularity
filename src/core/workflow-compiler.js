 ...
 _buildWorkflowContext(request, context) {
  ...
  switch (context.type) {
    case 'initialize':
      ...
      sessionId = request.sessionId || `session-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      sessionCreated = true;
      break;
  ...
  switch (request.type) {
    case 'initialize':
      if (!request.userMessage?.trim()) throw new Error('[Compiler] Initialize: userMessage required');
      if (!request.providers?.length) throw new Error('[Compiler] Initialize: providers required');
      break;
    
    case 'extend':
      if (!request.sessionId) throw new Error('[Compiler] Extend: sessionId required');
      if (!request.userMessage?.trim()) throw new Error('[Compiler] Extend: userMessage required');
    ...
    case 'recompute':
      ...
      if (!request.targetProvider) throw new Error('[Compiler] Recompute: targetProvider required');
      break;
  }
  }
  ...
  _validateContext(context) {
    ...
    switch (context.type) {
      ...
      case 'recompute': {
        ...
        if (!context.targetProvider) throw new Error('[Compiler] Recompute: targetProvider required');
        break;
      }
    }
  }
